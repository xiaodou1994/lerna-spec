 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-lerna-spec/index
 * @returns eslint-config-lerna-spec/react
 * @returns eslint-config-lerna-spec/typescript/index
 * @returns eslint-config-lerna-spec/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'eslint-config-lerna-spec/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
